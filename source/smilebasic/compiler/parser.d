...
class Parser {
  ...
  private {
    ...
    //UnaryOperators
    Node negExpr(ParseTree tree) const {
      return new UnaryOpNode(UnaryOp.Neg, node(tree.children[0]));
    }

    Node notExpr(ParseTree tree) const {
      return new UnaryOpNode(UnaryOp.Not, node(tree.children[0]));
    }

    Node logicalNotExpr(ParseTree tree) const {
      return new UnaryOpNode(UnaryOp.LogicalNot, node(tree.children[0]));
    }
    ...
    Node factor5(ParseTree tree) const {
      ...
      foreach (t; tree.children[1 .. $]) {
        temp = new BinaryOpNode((name) {
          switch (name) {
          case "Parser.leftShiftExpr":
            return BinaryOp.LShift;
          case "Parser.rightShiftExpr":
    ...
    Node factor6(ParseTree tree) const {
      ...
      foreach (t; tree.children[1 .. $]) {
        temp = new BinaryOpNode((name) {
          ...
          case "Parser.eqExpr":
            return BinaryOp.Eq;
          case "Parser.notEqExpr":
            return BinaryOp.NotEq;
          case "Parser.lessExpr":
            return BinaryOp.Less;
          case "Parser.greaterExpr":
            return BinaryOp.Greater;
          case "Parser.lessEqExpr":
            return BinaryOp.LessEq;
          case "Parser.greaterEqExpr":
    ...
    Node factor8(ParseTree tree) const {
      ...
      foreach (t; tree.children[1 .. $]) {
        temp = new BinaryOpNode((name) {
          switch (name) {
          case "Parser.logicalAndExpr":
            return BinaryOp.LogicalAnd;
          case "Parser.logicalOrExpr":
    ...
    Node variableDefineStatement(ParseTree tree) const {
      VariableNode[] defines = tree.children[].map!((a) {
      ...
      Node[] temp = tree.children[].filter!(a => a.name == "Parser.assignStatement")
        .map!(a => node(a)).array;
    ...
    Node ifThenStatement(ParseTree tree) const {
      if (tree.children.length == 1) {
        ...
        auto temp = tree.children[1 .. $].map!(a => node(a)).array;
    ...
    Node thenStatement(ParseTree tree) const {
      auto temp = tree.children.map!(a => node(a)).array;
    ...
    Node print(ParseTree[] trees) const {
      Node[] temp = trees.filter!(a => !(a.name == "Parser.commandDelimiter"
          && a.matches.front == ";")).map!(a => a.name != "Parser.commandDelimiter"
          ? node(a) : new ValueNode("\t"w)).array;
