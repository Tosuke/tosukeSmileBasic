...
mixin template OperatorMixin() {
  ...
  Value opUnary(string op : "-")() const {
    ...
    case ValueType.Integer:
      return Value(-(data.get!int));
    case ValueType.Floater:
      return Value(-(data.get!double));
    default:
      throw imcompatibleTypeError("-", this);
    }
  }
  ...
  int opCmp(Value b) const {
    if (this.isArithmeticValue && b.isArithmeticValue) {
      if (this.type == ValueType.Integer && b.type == ValueType.Integer) {
        immutable m = this.get!int;
        immutable n = b.get!int;
        if (m < n) {
          return -1;
        }
        else if (m == n) {
        ...
        immutable m = this.toFloater;
        immutable n = b.toFloater;
        if (m < n) {
          return -1;
        }
        else if (m == n) {
      ...
      immutable m = this.get!wstring;
      immutable n = b.get!wstring;
      if (m < n) {
        return -1;
      }
      else if (m == n) {
